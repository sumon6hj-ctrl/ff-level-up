...
WAIT_AFTER_MATCH_SECONDS = 8   
START_SPAM_DELAY = 0.2         

PROMO_TEXT = "TELEGRAM: BrExeee | INSTAGRAM: _kaiilash__"
...
def restart_program():
    ...
    # Web server me hum poora program restart nahi kar sakte, isliye pass kar diya
    pass
...
class FF_CLIENT(threading.Thread):
    def __init__(self, uid, password):
        ...
        # Thread automatically start ho jayega
        self.start()

    def run(self):
        # Thread start hone par ye function chalega
        self.get_tok()
    ...
    def connect(self, tok, whisper_ip, whisper_port, online_ip, online_port):
        ...
        while True:
            try:
                ...
                # ------- /exe COMMAND (Formerly /lw) -------
                if b"/exe" in data:
                    try:
                        json_result = get_available_room(data.hex()[10:])
                        if not json_result:
                            logging.warning("get_available_room returned None for /exe")
                            continue
                        ...
                        if uid is None:
                            logging.warning("UID not found in parsed_data for /exe")
                            continue

                        split_data = re.split(rb"/exe", data, maxsplit=1)
                        if len(split_data) < 2:
                            msg = "[C][B][FF0000]Please provide a team code after /exe."
                            msg += f"\n{PROMO_TEXT}"
                            clients.send(self.GenResponsMsg(msg, uid))
                            continue

                        # text after /exe, before '(' and newline
                        cmd_text = (
                            split_data[1]
                            .split(b"(")[0]
                        ...
                        if not team_code.isdigit():
                            msg = (
                                ...
                                "Please use like /exe123456 (only numbers)."
                            )
                            msg += f"\n{PROMO_TEXT}"
                            clients.send(self.GenResponsMsg(msg, uid))
                            continue
                        ...
                        logging.error(f"An error occurred in /exe command: {e}", exc_info=True)
                        try:
                            json_result = get_available_room(data.hex()[10:])
                            if json_result:
                                parsed_data = json.loads(json_result)
                                ...
                                if uid:
                                    msg = (
                                        "[C][B][FF0000]Something went wrong in /exe. "
                                        "Please use format: /exe123456 (numbers only)."
                                    )
                                    msg += f"\n{PROMO_TEXT}"
                                    clients.send(self.GenResponsMsg(msg, uid))
                        except Exception:
                ...
                if b"/help" in data:
                    try:
                        ...
                        # Part 1: Headline (White, Bold, Center)
                        msg1 = (
                            "[B][C]CHOOSE LONE WOLF MAP & SELECT DUEL MODE"
                        )
                        clients.send(self.GenResponsMsg(msg1, uid))
                        time.sleep(0.7)

                        # Part 2: Commands (White, Bold, Center)
                        msg2 = (
                            "[B][C]THEN USE COMMAND: \n/exe{Teamcode}\n"
                            "EXAMPLE:- /exe123456"
                        )
                        clients.send(self.GenResponsMsg(msg2, uid))
                        time.sleep(0.7)

                        # Part 3: Socials (White, Bold, Center)
                        msg3 = (
                            "[B][C]FOLLOW ME IF YOU GET WORK\n"
                            "TELEGRAM: BrExeee\n"
                            "INSTAGRAM: _kaiilash__"
                        )
                        clients.send(self.GenResponsMsg(msg3, uid))
                        time.sleep(0.7)
                        
                        # Part 4: Owner (White, Bold, Center)
                        msg4 = (
                            "[B][C]OWNER TG: @Brexxee"
                        )
                        clients.send(self.GenResponsMsg(msg4, uid))
    ...
    def get_tok(self):
        ...
        if not token_data:
            logging.critical("Failed to get token data from guest_token.")
            return
...
if __name__ == "__main__":
    print("Do not run this file directly. Run main.py instead.")
